...
def restore_backup(backup_folder: str, restore_source_code: bool = False) -> dict:
    """
    Restores JSON databases, verifies checksums, and optionally restores application source.
    """
    ...
    # 4. Copy live SQLite DB to project root for instant querying
...
def interactive_menu():
    ...
    print("\nOptions:")
    print("  [1] Quick Restore Latest JSON & SQL Databases (Recommended)")
    print("  [2] Quick Restore Latest FULL (Databases + App Source Code)")
    ...
    if choice in ("", "1"):
        restore_backup(backups[0], restore_source_code=False)
    elif choice == "2":
        restore_backup(backups[0], restore_source_code=True)
    elif choice == "3":
        ...
        try:
            ...
            if 0 <= chosen_idx < len(backups):
                ...
                restore_backup(backups[chosen_idx], restore_source_code=(full_c == "y"))
            else:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if arg in ("--list", "-ls"):
            ...
            backups = find_available_backups()
            if backups:
                restore_backup(backups[0], restore_source_code=include_code)
            else:
                print("No backups found.")
        else:
            restore_backup(arg, restore_source_code=include_code)
    else:
        interactive_menu()
